...
| **Gus Grokman** | Vehicle Engineering Lead | `.craft` proposals (many per hire), `capable:` on vehicle tickets (**FED**: `craft fuel`). Hand-typed PART blocks are last resort — file `vab-helper` at Wernher | Hangar, fly, `.py` |
...
| **Wernher Grokman** | Chief Systems Engineer | Software/world architecture: kRPC, desk, hangar scenes, telem schema, ops kernel, protocol; **control blocks** (`physics_warp.py` sit/warp/timeout/leftover/chute, `rf_throttle.py` RF live catalog) | this-hop pulse (`ascent.py` / hop_factory), `.craft` |
| **Lars Grokman** | Vehicle Systems Engineer | How the vehicle is *flown this sit*: **one living rocket's pulse** composed from Wernher blocks (`ascent.py` orbit / `python main.py ascent`, `hop_factory.py` inland, or a t7-only file; **pad-RF** is `hop_factory_pad.py` — one sit, no `_pad_*` per stamp), pad/splash, this-hop splash HD | Leftover recover-then-Hangar (Hank/Wernher), warp *law*, `rf_throttle.py`, stamp-named helpers, immortal `hop.py` factory, org, Hangar from Gene |
...
| **Eleanor Grokman** | Director of Constellation Operations | RealAntennas, Cape/ground, future crafts / sats; rare asks | `.py`, kRPC Session, press, windows, CAPCOM |
...
`docs/program/krpc.md` (Gene / Lars / Gus / Katherine / Eleanor / Hank).
...
`--tag dynamics` / `ops --tag ask --desk katherine`. Eleanor only if
`--tag constellation` / `ops --tag ask --desk eleanor`. Gene does not
...
- Eleanor may last-write **Constellation** on `agree.md` when pulled (`--tag constellation` / `ops --tag ask --desk eleanor`).
...
if leftover (desk hangar recover/blocked, live probe, crash UI):
    ...
    # Walk home: recover() the ship and Close to KSC. Persist must
    # survive a split wreck; skip-dup is persist not the broom.
...
| Open `desk=eleanor` / `--tag constellation` | **Eleanor** | that ops ticket — Cape / RA / ground / future crafts. Not every `ops next`. Disk only. Reports Wernher asks |
...
tickets that turn. Katherine only if the plan names her. Eleanor only
if the plan names her (`--tag constellation` / `--desk eleanor`). Hank
