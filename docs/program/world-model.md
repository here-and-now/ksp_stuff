...
| Stem | Count | Pitfall |
|---|---|---|
| `tui-reasoning` | 1 (T-540) | Job cards injected `Reasoning is **medium\|high**`. Hank copied `ops next` `reasoning=` into spawn packets. Everyone inherits **current TUI reasoning**. Hank does **not** copy `reasoning=`. Kernel still prints `reasoning=` (`ops.py` `format_next`) / packet `reasoning:` (`tickets.py` `format_packet`) / AGENTS `Copy reasoning=` — Wernher systems. Eleanor Grokman, Director of Constellation Operations: RA / Cape / future crafts; `ops --tag ask --desk eleanor` / `--tag constellation`. `DESKS` / `crew.py` / `ops.py` hire lists / `protocol.py` SCHEMAS / AGENTS spawn table are kernel — same Wernher ticket. |
