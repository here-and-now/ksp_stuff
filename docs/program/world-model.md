...
| `vab-helper` | 16 (T-496 ×14) | A hang you cannot prove is **FED** is not capable. `craft fuel` dumps attach + `fuelCrossFeed` (T-495). `insert_heatshield` refuses `fuelCrossFeed=False` (T-497). C-477 last-tank→dish HS→Valiant Ablator-only starved (pad Δv 0/0). Engine first fire (`sqor=0`) and HS VAB dish still hold. Do not write GameData. Pad flies a fed hang — do not idle. |
