...
| **capable:** | Gus sign-off. Keep. **FED** proof (`python main.py craft fuel <craft>`) is part of the stamp: tanks must reach the engine. Loft hang this sit is **C-534** (FED, no HS, no girders; recover silk). C-504 loft-pbc stays shelf. Starved / blob / Hangar-detonating HS is `capable: no` (C-477). FED is not enough: a Hangar-detonating HS splice is `capable: no` (T-500). |
...
| **CAPCOM** | NASA loop. **Walt**. TUI is phase start / phase end / unexpected only. Unexpected includes engine-dead (thrust 0 / plume no / fuel frozen / parts intact) — not last-flight shear. Verena is Communications. Keep the split. |
| **Hangar** | KSP building + our launch helper. Keep. |
| **leftover / KSC** | Space Center wreck or unmatched vessel (includes recoverable **ground Debris** — pad Goo). **Hank** walk home (lock free, rec=yes or crash UI): `recover()` if recoverable, else Close (`recover-probe --recover` when recoverable). `recover()` is persist-then-KSC; Python RPC returning is not despawn. Wait GUID on `vessels`, not leftover names. Commander hop does not recover leftover. This-hop splash HD after a briefed dwell is mission. Never revert. Airborne rec=0 is occupancy — see leftover-while-flying. |
| **leftover-ksc** | **Retired as a load.** Named save/load of the overlay looked like a reload / return to pre-launch. Persist must survive a split wreck. Harmony skip-dup is persist fail-open, not the broom. Flying rec=0 blobs that throw Kerbalism persist make recover a no-op — quit KSP that sit. Overlay dismiss is `recover()` + Close. Hop timeout leftover is leftover_call (recover vs ksc leftover) once down or never lofted, not a novel emergencies.call string. `ksc leftover` is the registered leftover abort verb. Never that load. Do not reuse this stem as a grab-bag. |
| **leftover-while-flying** | Lofted timeout while flying rec=no. Occupancy, not leftover-ksc, not leftover-prelaunch-ghost. Silk/coast until down+recoverable; then leftover_call recover vs ksc leftover. Do not Close while lofted rec=no. Does not alias onto leftover-ksc. |
...
| **inner circle** | Lars + Gus + Linus. Confer on `agree.md` (`ops --tag plan`), then split. Gene is not this merge. Katherine opt-in. Eleanor opt-in. |
...
| **comms** | Eleanor (network map) / Gus (antenna on the hang). Disk dump is ConfigCache (TL rates, craft, ground LIVE/SILENT). Live RA kRPC is `conn.real_antennas` (Os 2026-08-25). Do not cheat a link. |
...
| **Constellation Operations** | Eleanor. RealAntennas, Cape/ground, future crafts / sats. Disk only. Reports Wernher asks. Not press, not windows, not CAPCOM. Opt-in: `ops --tag ask --desk eleanor` / `--tag constellation`. |
