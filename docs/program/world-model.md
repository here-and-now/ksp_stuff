...
| `rf-ignition-ullage` | 10 (T-470 ×8) | Hop light is **plume**, not ignitions 1→0. kRPC GET throttle is not the burn. Independent setpoint is the RF live. Empty stage is not hop light. Pad-dead-no-plume on a **fed** engine is Lars `hop_factory_pad.py`. Airborne cutoff throttle 1 + thrust 0 + fuel frozen + parts intact is this stem (T-509), not shear. Starved Δv 0/0 is `craft fuel` / vab-helper, not RF. Pad 1 g still lights; restart is not free; do not raise ignitions. T-471 owns pad-dead; T-509 owns loft cutoff. |
