...
| **capable:** | Gus sign-off. Keep. **FED** proof (`python main.py craft fuel <craft>`) is part of the stamp: tanks must reach the engine. Loft hang this sit is **C-504** (FED, no HS/chute). Starved / blob / Hangar-detonating HS is `capable: no` (C-477). FED is not enough: a Hangar-detonating HS splice is `capable: no` (T-500). |
