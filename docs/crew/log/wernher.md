...
- 2026-08-26 — T-479 Close uniquify: Kerbalism OnSave Key 1465015097 is VesselData parts.Add(flightID) when wreck + `<craft> Debris` share basename (Cape rec=yes blob). Names uniquified before persist; rec=yes ground debris is leftover. Never leftover-ksc. Never revert.
